...
type RichMap3 =
  ...
  static member pixmapGenerator (rich : RichMap3) =
    { new IPixmapSource with
      member __.start(w, h) = ()
      member __.getPixel(w, h, x, y) =
        rich.map (Vec3f(float32 x / float32 w, float32 y / float32 h, 0.0f)) * 0.5f + Vec3f(0.5f)
      member __.finish() = ()
  ...
  static member generate (mapGenerator : Dna -> Map3) (dna : Dna) =
    ...
    let invZoom = 1.0f / zoom
    let map = mapGenerator dna
    let info = Map3Info.create(map)
    {
      RichMap3.map = (fun (v : Vec3f) -> v * invZoom + offset) >> map >> info.normalizer >> permute permutation >> palette
      center = center
      zoom = zoom
