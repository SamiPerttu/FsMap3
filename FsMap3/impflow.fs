...
let impflow (layout : LayoutFunction) (count : FeatureCount) (potential : Potential3) (mix : MixOp) (color : CellColor) (fade : float32 -> float32) (shading : float32) (radius : float32) (flow : Basis3) (flowFrequencyFactor : float32) (frequency : float32) (v : Vec3f) =
...
let inline impflowd fade potential radius flow flowFF frequency v = impflow hifiLayout unityCount potential Mix.sum anyColor fade 0.5f radius flow flowFF frequency v
...
let patflow (layout : LayoutFunction) (count : FeatureCount) (potential : Potential3) (mix : MixOp) (fade : float32 -> float32) (radius : float32) (curvature : float32) (pattern : Atlas3) (flow : Basis3) (flowFrequencyFactor : float32) (frequency : float32) (v : Vec3f) =
  // Get rotation from the flow map.
  let L = G pi * flow (frequency * flowFrequencyFactor) v
  let length = L.length
  let pose = if length > 1.0e-9f then Quaternionf(L / length, length) else Quaternionf.one
