...
let capflow (layout : LayoutFunction) (count : FeatureCount) (mix : MixOp) (color : CellColor) (fade : float32 -> float32) (shading : float32) (length : float32) (radius : float32) (flow : Basis3) (flowFrequencyFactor : float32) (frequency : float32) (v : Vec3f) =
      ...
      for jz = data.z0 to data.z1 do
        ...
        for __ = 1 to count h do
          ...
          if distance2 < R2 then
            ...
            value <- mix value w (color h * (1G + S * (Ri * shading)))
  data.release()
  Mix.result value
