...
let peacock (layout : LayoutFunction) (count : FeatureCount) (potential : Potential3) (mix : MixOp) (color : CellColor) (fade : float32 -> float32) (shading : float32) (radius : float32) (frequency : float32) (v : Vec3f) =
  assert (radius > 0G)
...
let inline peacockd potential radius frequency v = peacock hifiLayout unityCount potential Mix.sum anyColor Fade.smooth 0G radius frequency v


/// Peacock pattern with the standard cell layout.
let inline peacockf potential fade radius frequency v = peacock hifiLayout unityCount potential Mix.sum anyColor fade 0G radius frequency v
