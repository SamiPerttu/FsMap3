...
let valueBinner (bins : int) (dna : Dna) i =
  ...
  if parameter.format = ParameterFormat.Ordered && parameter.range > uint64 bins then
...
let runHash (hash : Hash128) dna i (parameterHash : ParameterHash) =
  hash.reset()
